...
const USERS = [
  { username: 'admin', password: 'totoy123' },
  { username: 'admin', password: 'ced123'   },
];
...
// ── Login ──────────────────────────────────────────────────
function checkLogin() {
  const loggedIn = sessionStorage.getItem('mp_auth');
  if (!loggedIn) {
...
function setDate() {
  ...
  if (el) el.textContent = new Date().toLocaleDateString('en-PH', { weekday: 'long', month: 'long', day: 'numeric' });
}
...
async function deleteListing(id) {
  ...
  if (useCloud) await db.from('listings').delete().eq('id', id);
...
function renderDashboard() {
  ...
  funnelChart = new Chart(document.getElementById('funnelChart'), {
    ...
    data: {
      ...
      datasets: [{ data: [total, negot, closed, lost], backgroundColor: ['#B5D4F4','#FAC775','#C0DD97','#F7C1C1'], borderRadius: 5, borderSkipped: false }]
    },
  ...
  revenueChart = new Chart(document.getElementById('revenueChart'), {
    ...
    data: {
      labels: revLabels,
      datasets: [{ label: 'Revenue', data: revByMonth.map(v => Math.round(v)), borderColor: '#1a1a18', backgroundColor: 'rgba(26,26,24,0.05)', fill: true, tension: 0.35, pointRadius: 4, pointBackgroundColor: '#1a1a18' }]
    },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: v => '₱' + Math.round(v).toLocaleString('en-PH') } }, x: { grid: { display: false } } } }
  });
  ...
  recentEl.innerHTML = !recent.length
    ? '<div style="text-align:center;padding:1.5rem;color:var(--text-3);font-size:13px;">No leads yet.</div>'
    : recent.map(l => `
